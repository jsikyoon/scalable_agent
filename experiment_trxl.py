...
# Transformer
flags.DEFINE_enum('init', 'normal', ['normal', 'uniform'],
...
flags.DEFINE_integer('n_layer', 12, 'the number of layers of transformer')
flags.DEFINE_float('dropout', 0.0, 'dropout rate for linear layer')
flags.DEFINE_float('dropatt', 0.0, 'dropout rate for attention layer')
flags.DEFINE_integer('mem_len', 512, 'the length of memory')
flags.DEFINE_bool('pre_lnorm', False, 'Doing layer norm before or after')
...
class Agent(snt.RNNCore):
  ...
  def unroll(self, actions, env_outputs, core_state):
    ...
    initial_core_state = tf.zeros([tf.shape(actions)[1],
                                   FLAGS.mem_len,
                                   FLAGS.n_layer,
                                   256], tf.float32)
    core_output_list = []
    for input_, d in zip(tf.unstack(torso_outputs), tf.unstack(done)):
      ...
      core_state = nest.map_structure(functools.partial(tf.where, d),
                                      initial_core_state,
                                      core_state)
      core_state = tf.transpose(core_state, perm=[2,1,0,3])
      #core_output, core_state = self._core(input_, core_state)
      with tf.variable_scope('linear_before_transformer'):
        input__ = tf.nn.relu(snt.Linear(256)(input_)) # make f_dim to 256
      core_output, core_state = trxl(dec_inp=tf.expand_dims(input__, axis=0),
                                     mems=core_state,
